...
rm -r ${dest_path_data}
mkdir -p ${dest_path_data}  # initialize empty directory to remove any possible duplication
...
find /disk/scratch/s2089883/CycleGAN_ref/pytorch-CycleGAN-and-pix2pix/scripts/checkpoints/style_monet_pretrained/test_latest/images/ -type f -name "*real.png" -delete

echo "moving fake images to ${SCRATCH_HOME}/Monet/${BATCH}/"
mkdir -p ${SCRATCH_HOME}/Monet/${BATCH}
rsync -ua --progress /disk/scratch/s2089883/CycleGAN_ref/pytorch-CycleGAN-and-pix2pix/scripts/checkpoints/style_monet_pretrained/test_latest/images/ ${SCRATCH_HOME}/Monet/${BATCH}/
rsync -ua --progress /home/${STUDENT_ID}/Code/Cluster/CycleGAN_Monet/PickleAugmented4.py ${SCRATCH_HOME}/Monet/

ls ${SCRATCH_HOME}/Monet/

echo "Pickling fake images"
cd ${SCRATCH_HOME}/Monet/
python PickleAugmented4.py
...
rsync --archive --update --compress /disk/scratch/s2089883/CycleGAN_ref/pytorch-CycleGAN-and-pix2pix/scripts/checkpoints/style_monet_pretrained/test_latest/images/ /home/${STUDENT_ID}/Code/Cluster/data/Raw/Monet/${BATCH}
