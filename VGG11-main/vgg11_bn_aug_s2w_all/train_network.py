...
#trial - no transforms
    ...
    trainset = data_providers.CIFAR100(root='data', set_name='train', download=False, transform=transform_train)
    styled = data_providers.AugmentedCIFAR10(path='./data/S2W',transform=transform_train)

    concatenated = torch.utils.data.ConcatDataset([trainset, styled])
    print('Total elements for training:',len(concatenated))

    train_data = torch.utils.data.DataLoader(concatenated, batch_size=100, shuffle=True, num_workers=4)

    valset = data_providers.CIFAR100(root='data', set_name='val', download=False, transform=transform_test)
